...
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/it-business-management/time-card-management/submit-time-card.html
release: zurich
...
When a user submits a time card, the [time sheet policy](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/it-business-management/time-card-management/time-sheet-policies.md) assigned to the user is used for validation. If there’s no time sheet policy assigned to the user, the default time sheet policy is used. Business rules help prevent a user from submitting a time card in case any violation is found.
