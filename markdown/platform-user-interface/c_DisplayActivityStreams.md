...
The activity stream information appears in a flyout window, and is the same information that appears in the [activity formatter](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/platform-administration/ai-platform-administration/c_ActivityFormatter.md) for a record. The short description appears as the title for the entry. Activity stream items on the task table use the task number as the title. The information in the flyout window updates automatically with audit and journal entries. Click the **&gt;&gt;** icon at the top to close the activity stream.
