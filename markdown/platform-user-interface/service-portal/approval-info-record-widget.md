---
title: Approval Info Record widget
description: The Approval Info Record widget shows details about the approval request and a full record for an approval including the activity stream.
locale: en-US
canonical_url: https://www.servicenow.com/docs/r/zurich/platform-user-interface/service-portal/approval-info-record-widget.html
release: zurich
product: Service Portal
classification: service-portal
topic_type: concept
last_updated: "2025-10-30"
reading_time_minutes: 1
breadcrumb: [Example widgets, Widget library, Using portal widgets, Configuring Service Portal, Service Portal, Configure UIs and portals, Configure user experiences]
---

# Approval Info Record widget

The Approval Info Record widget shows details about the approval request and a full record for an approval including the activity stream.

## Using the widget

\[Omitted image "widgetapprovaldetails.png"\] Alt text: Approval Info Record widget showing all of the information available about the approval including comments by the requester.

Configure the field values and the order of the fields from Service Portal Approval Configuration. For more information, see [Configure Service Portal Approval Configuration record](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/platform-user-interface/service-portal/configure-approval-assistance-ai-agent.md).

## Instance options

|Field|Description|
|-----|-----------|
|Bootstrap color|Color scheme for the widget. The default colors are defined by the portal theme, but if you want the instance to have a specific color, select the option from the list.|

**Parent Topic:**[Example widgets](https://raw.githubusercontent.com/ServiceNow/ServiceNowDocs/zurich/markdown/zurich/platform-user-interface/service-portal/sp-example-widgets.md)

